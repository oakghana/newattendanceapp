...
export default async function DriverLicensesPage() {
  ...
  return <DriverLicenseWorkspace initialDrivers={isDriver ? (ownDriver ? [ownDriver] : []) : (drivers ?? [])} canVerify={!isDriver && (isRegionalHrRole(profile.role) || normalizedRole === "transport_manager")} role={isDriver ? "driver" : normalizedRole} assignedTasks={driverTasks ?? []} />
}
