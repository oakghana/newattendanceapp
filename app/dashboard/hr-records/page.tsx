        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[760px] text-left text-sm">
            ...
            <tbody className="divide-y">
              {rows.map((row) => { const locked = Boolean(row.memo_reference_locked); return <tr key={`${row.entity}-${row.id}`}><td className="px-5 py-4 font-medium">{row.label}</td><td className="px-5 py-4 capitalize">{row.entity}</td><td className="px-5 py-4 text-muted-foreground">{row.status === "pending_hr_records_reference" || row.workflow_stage === "pending_hr_records_reference" ? "Awaiting HR Records memo reference" : row.status === "pending_hr_leave_processing" || row.workflow_stage === "pending_hr_leave_processing" ? "Awaiting HR Leave Office adjustment" : row.workflow_stage || row.status || "Pending"}</td><td className="px-5 py-4"><input aria-label={`Official reference for ${row.label}`} defaultValue={row.reference || ""} disabled={saving === row.id} className="w-full rounded-md border bg-background px-3 py-2 outline-none focus:ring-2 focus:ring-ring disabled:cursor-not-allowed disabled:bg-muted" /></td><td className="px-5 py-4 text-muted-foreground">{row.memo_reference_locked_at ? new Date(row.memo_reference_locked_at).toLocaleString() : "Not recorded"}</td><td className="px-5 py-4"><button type="button" disabled={saving === row.id} onClick={(event) => { const input = event.currentTarget.parentElement?.previousElementSibling?.querySelector("input"); if (input) void saveReference(row.entity, row.id, input) }} className="rounded-md bg-primary px-3 py-2 font-medium text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50">{saving === row.id ? "Saving…" : locked ? "Update reference" : "Save & forward"}</button></td></tr> })}
