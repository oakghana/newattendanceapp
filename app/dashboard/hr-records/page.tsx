...
const overdueRows = useMemo(() => [...data.leave, ...data.loans, ...data.transport].filter((row) => {
    // The aging notice must use the same eligibility rules as Pending references.
    // In particular, a loan's provisional reference_number is not an HR Records
    // reference and must not make an unrelated queue row appear overdue.
    if (!canEditReference(row) || isReferenceLocked(row)) return false
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[760px] text-left text-sm">
            ...
            <tbody className="divide-y">
                            {rows.map((row) => { const locked = isReferenceLocked(row); const ageDate = new Date(row.updated_at || row.submitted_at || row.created_at || 0); const overdue = !locked && ageDate.getTime() > 0 && Date.now() - ageDate.getTime() > 3 * 24 * 60 * 60 * 1000; return <tr key={`${row.entity}-${row.id}`} className={overdue ? "bg-red-50 text-red-950" : undefined}>
