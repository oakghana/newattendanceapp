...
export default function HrRecordsPage() {
  ...
  const rows = useMemo(() => [
    ...
    const regionalStatuses = ["pending_regional_hr_office_review", "pending_regional_hr_review", "regional_hr_office_review", "regional_hr_approved"]
    const isRegionalLeave = row.entity === "leave" && regionalStatuses.includes(String(row.status || "").toLowerCase())
    return view === "pending" ? (!row.memo_reference_locked || isRegionalLeave) : view === "referenced" ? Boolean(row.memo_reference_locked) && !["hr_approved", "approved"].includes(row.status) : ["hr_approved", "approved"].includes(row.status)
  }), [data, view])
