  ...
  try {
    ...
    if (updateError) {
      ...
      if (
        ...
        updateMessage.includes("value too long") ||
        updateCode === "23514" ||
        updateCode === "22001"
      ) {
        const safeSuggestedSQL = `-- Replace <constraint_name> if different. Example uses ${constraintName || 'user_profiles_role_check'}\nALTER TABLE user_profiles DROP CONSTRAINT IF EXISTS ${constraintName || 'user_profiles_role_check'};\nALTER TABLE user_profiles ADD CONSTRAINT ${constraintName || 'user_profiles_role_check'} CHECK (role IN ('admin','it-admin','department_head','regional_manager','nsp','intern','contract','staff','audit_staff','accounts','loan_office','hr_office','hr_leave_office_admin','hr_leave_office','regional_hr_leave','director_hr','manager_hr','loan_committee','committee'));
