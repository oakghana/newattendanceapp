import { NextRequest, NextResponse } from 'next/server';
import { createAdminClient } from '@/lib/supabase/server';

/**
 * GET /api/loan/regional-office/loans
 * Regional Loan Office can view all loan requests from their assigned region/location
 * Cannot approve/reject, only view and export
 */
export async function GET(request: NextRequest) {
  try {
    const admin = await createAdminClient();
    const userId = request.headers.get('x-user-id');

    if (!userId) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }

    // Get user profile to check role and assigned locations
    const { data: userProfile, error: profileError } = await admin
      .from('user_profiles')
      .select('id, role, assigned_location_id')
      .eq('id', userId)
      .single();

    if (profileError || !userProfile) {
      return NextResponse.json({ error: 'User not found' }, { status: 404 });
    }

    // Regional Managers can view and print loans approved within their assigned location.
    if (!['regional_loan_office', 'regional_manager', 'admin'].includes(String(userProfile.role || '').toLowerCase())) {
      return NextResponse.json({ error: 'Insufficient permissions' }, { status: 403 });
    }

    // Get the assigned locations for this regional loan office
    const { data: assignedLocations, error: locationsError } = await admin
      .from('regional_loan_office_locations')
      .select('location_id, region_id')
      .eq('regional_loan_office_id', userId)
      .eq('is_active', true);

    if (locationsError) {
      console.error('[v0] Error fetching assigned locations:', locationsError);
      return NextResponse.json(
        { error: 'Failed to fetch assigned locations' },
        { status: 500 }
      );
    }

    const locationIds = assignedLocations?.map(loc => loc.location_id) || [];
    if (locationIds.length === 0 && userProfile.assigned_location_id) {
      locationIds.push(userProfile.assigned_location_id)
    }

    if (locationIds.length === 0) {
      return NextResponse.json(
        { loans: [], summary: { total: 0, pending: 0, approved: 0 } },
        { status: 200 }
      );
    }

    // Fetch loan requests for assigned locations
    const { data: loans, error: loansError, count } = await admin
      .from('loan_requests')
      .select(
        `
        id,
        request_number,
        user_id,
        staff_number,
        staff_rank,
        loan_type_key,
        loan_type_label,
        requested_amount,
        fixed_amount,
        reason,
        status,
        submitted_at,
        created_at,
        updated_at,
        hod_decision,
        staff_location_id,
        recovery_months,
        disbursement_date
        `,
        { count: 'exact' }
      )
      .in('staff_location_id', locationIds)
      .order('created_at', { ascending: false });

    if (loansError) {
      console.error('[v0] Error fetching loans:', loansError);
      return NextResponse.json(
        { error: 'Failed to fetch loans' },
        { status: 500 }
      );
    }

    // Calculate summary statistics
    const summary = {
      total: count || 0,
      pending: loans?.filter(l => l.status === 'pending_hod').length || 0,
      approved: loans?.filter(l => l.status === 'hr_office_approved').length || 0,
      rejected: loans?.filter(l => l.status === 'rejected').length || 0,
      byType: {} as Record<string, number>,
    };

    // Group by loan type
    loans?.forEach(loan => {
      const key = loan.loan_type_label || 'unknown';
      summary.byType[key] = (summary.byType[key] || 0) + 1;
    });

    return NextResponse.json({
      success: true,
      loans,
      summary,
      message: 'Regional loan office data fetched successfully',
    });
  } catch (error) {
    console.error('[v0] Regional loan office API error:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
  }
}
