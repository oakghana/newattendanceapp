...
export async function GET() {
  ...
  const [leaveResult, loanResult] = await Promise.all([
    admin.from("leave_plan_requests").select("id, user_id, leave_type_key, status, workflow_stage, workflow_route, memo_reference, memo_reference_locked, memo_reference_locked_at, memo_reference_locked_by, submitted_at, updated_at, preferred_start_date, preferred_end_date, hr_approved_at, reason, subject").in("status", ["pending_hr_records_reference", "hod_approved", "hr_office_forwarded", "pending_hr_leave_processing", "regional_manager_approved", "hr_approved", "approved"]).order("updated_at", { ascending: false }),
    admin.from("loan_requests").select("id, user_id, request_number, reference_number, status, workflow_stage, memo_reference_locked, memo_reference_locked_at, submitted_at, updated_at, subject, purpose").in("status", ["pending_hr_records_reference", "hod_approved", "approved_director"]).order("updated_at", { ascending: false }),
  ])
  ...
  const { data: profiles, error: profilesError } = userIds.length
    ? await admin.from("user_profiles").select("id, first_name, last_name, employee_id, role, department, assigned_location_id").in("id", userIds)
    : { data: [], error: null }
  ...
  const decorate = (row: any) => {
    ...
    return {
      ...
      request_subject: row.subject || row.purpose || row.reason || row.leave_type_key || row.request_number || "Staff request",
    }
  }
