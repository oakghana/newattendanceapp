...
export async function GET() {
  ...
  const [leaveResult, loanResult] = await Promise.all([
    ...
    admin.from("loan_requests").select("id, user_id, request_number, reference_number, status, workflow_stage, memo_reference_locked, memo_reference_locked_at, submitted_at, updated_at, purpose").in("status", ["pending_hr_records_reference", "hod_approved", "approved_director"]).order("updated_at", { ascending: false }),
  ])
  ...
  const decorate = (row: any) => {
    ...
    return {
      ...
      request_subject: row.purpose || row.reason || row.leave_type_key || row.request_number || "Staff request",
    }
  }
