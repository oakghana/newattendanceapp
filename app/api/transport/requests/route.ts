...
const update = isManagingDirector
  ...
  else if (decision === "approve_hr_memo") { if (!row.memo_subject || !row.memo_body) return NextResponse.json({ error: "Open and save the edited memo before approving it." }, { status: 409 }); const { data: signer } = await supabase.from("user_profiles").select("signature_data_url, first_name, last_name, position").eq("id", user.id).single(); const signerName = `${signer?.first_name ?? ""} ${signer?.last_name ?? ""}`.trim(); update = { status: "approved", workflow_stage: row.request_type === "regional_transport" ? "hr_records_review" : "transport_manager_assignment", hr_executive_signer_id: user.id, hr_executive_signed_at: new Date().toISOString(), hr_executive_signature_data_url: signer?.signature_data_url ?? null, memo_amendments: JSON.stringify({ text: row.memo_amendments ?? "", hr_executive_signer_id: user.id, hr_executive_signed_at: new Date().toISOString(), hr_executive_signature_data_url: signer?.signature_data_url ?? null, hr_executive_signer_name: signerName || null, hr_executive_signer_position: signer?.position ?? "HUMAN RESOURCES MANAGER" }), hr_executive_handoff_by: user.id, hr_executive_handoff_at: new Date().toISOString(), updated_at: new Date().toISOString() } }
