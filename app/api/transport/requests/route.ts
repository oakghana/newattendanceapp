...
export async function PATCH(request: Request) {
  ...
  const { data: row } = await supabase.from("transport_requests").select("id, purpose, origin, destination, event_date, passenger_count, assigned_region_id, linked_district_id, origin_location_id, workflow_stage, memo_reference, memo_date, memo_subject, memo_body, memo_amendments, regional_manager_signer_id, regional_manager_signed_at, hr_records_amended_by, hr_records_amended_at, hr_executive_signer_id, hr_executive_signed_at, hr_executive_signature_data_url").eq("id", requestId).single()
  ...
  else if (decision === "endorse") update = { status: "endorsed", workflow_stage: "managing_director_approval", regional_manager_signer_id: user.id, regional_manager_signed_at: new Date().toISOString(), updated_at: new Date().toISOString() }
