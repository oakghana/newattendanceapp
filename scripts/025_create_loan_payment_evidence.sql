...
CREATE TABLE IF NOT EXISTS public.loan_payment_evidence (
  ...
  reference_number VARCHAR(255), -- Bank reference, cheque number, transaction ID, etc.
  ...
  submitted_by UUID NOT NULL REFERENCES public.auth.users(id), -- HR/Accounts staff who submitted
  submitted_at TIMESTAMP DEFAULT NOW(),
  
  -- HR Executive approval
  approved_by UUID REFERENCES public.auth.users(id), -- HR Executive who approved
  approved_at TIMESTAMP,
  approval_notes TEXT,
  
  -- Rejection details
  rejected_by UUID REFERENCES public.auth.users(id), -- HR Executive who rejected
