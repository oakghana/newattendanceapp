...
function isKnownLocation(name) {
  ...
  return v !== "unknown" && v !== "unknown location"
}
...
async function main() {
  ...
  console.log(`[force-checkout] Processing date: ${today}`)
  ...
  if (failErr) {
    console.error("[force-checkout] Failed to query audit logs:", failErr)
    process.exit(1)
  }
  ...
  for (const row of failures || []) {
    ...
    const attemptType = String(p.attempt_type || "")
    const nearestDistance = Number(p.nearest_location_distance_m)
    const nearestName = p.nearest_location_name || null

    // Only users attempting checkout from app within range (<=100m) and with known nearest location.
    if (!Number.isFinite(nearestDistance) || nearestDistance > 100) continue
    if (!isKnownLocation(nearestName)) continue
    ...
    if (attemptType) g.attempt_types.add(attemptType)
  ...
  const candidates = Array.from(grouped.values()).filter((x) => x.attempts >= 3)

  if (candidates.length === 0) {
    console.log("[force-checkout] No users matched criteria (>=3 failures, <=100m, known QCC location).")
    process.exit(0)
  }
  ...
  if (attErr) {
    console.error("[force-checkout] Failed to query open attendance:", attErr)
    process.exit(1)
  }
  ...
  for (const candidate of candidates) {
    ...
    const updatePayload = {
      ...
      notes: `Force checkout applied after ${candidate.attempts} failed checkout attempts while within range.`,
    ...
    await supabase.from("audit_logs").insert({
      ...
      new_values: {
        force_rule: ">=3 failed checkout attempts today within <=100m",
  ...
  console.log("[force-checkout] Completed")
...
main().catch((err) => {
  console.error("[force-checkout] Unexpected error:", err)
  process.exit(1)
})
