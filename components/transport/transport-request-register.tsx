...
export function TransportRequestRegister({ rows, canCreate, canAct, canHrRecords, canManagingDirector, canHrExecutive, regionalOfficeName }: { rows: TransportRequestRow[]; canCreate: boolean; canAct: boolean; canHrRecords: boolean; canManagingDirector: boolean; canHrExecutive: boolean; regionalOfficeName: string }) {
  ...
  const downloadMemo = async (row: TransportRequestRow) => { setMemoView("rejoinder"); let signature: string | null = row.hr_executive_signature_data_url ?? null; let signerName: string | null = null; let signerPosition: string | null = null; let regionalManagerComment: string | null = null; let hrAmendments = ""; try { const amendments = row.memo_amendments ? JSON.parse(row.memo_amendments) as Record<string, unknown> : {}; signerName = typeof amendments.hr_executive_signer_name === "string" ? amendments.hr_executive_signer_name : null; signerPosition = typeof amendments.hr_executive_signer_position === "string" ? amendments.hr_executive_signer_position : null; regionalManagerComment = typeof amendments.regional_manager_comment === "string" ? amendments.regional_manager_comment : null; hrAmendments = typeof amendments.text === "string" ? amendments.text : "" } catch {} if (row.hr_executive_signer_id) { const response = await fetch(`/api/user/signature/${row.hr_executive_signer_id}`); if (response.ok) signature = (await response.json()).signature_image_url ?? null } const eventDate = row.event_date ? new Date(`${row.event_date}T00:00:00`).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" }) : "the approved date"; const body = [`Management has approved transportation support for the ${row.purpose.toLowerCase()} in the ${row.destination} District Office.`, `The approved vehicle support is for the transportation of ${row.passenger_count} passengers from ${row.origin} to ${row.destination} on ${eventDate}.`, hrAmendments ? `Regional HR Office remarks: ${hrAmendments}` : "", regionalManagerComment ? `Regional Manager's approval comment: ${regionalManagerComment}` : "", "Kindly make the necessary arrangements to facilitate the approved transportation.", "You can count on our usual cooperation."].filter(Boolean).join("\n\n"); setMemoPreview({ ...row, hr_executive_signature_data_url: signature, hr_executive_signer_name: signerName, hr_executive_signer_position: signerPosition, memo_subject: `RE: ${(row.memo_subject || row.purpose).replace(/^\s*RE:\s*/i, "")}`, memo_body: body }) }
  const openMemoPreview = (row: TransportRequestRow, view: "original" | "rejoinder" = "rejoinder") => { void downloadMemo(row); setMemoView(view) }
