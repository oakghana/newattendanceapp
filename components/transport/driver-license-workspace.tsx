...
type MissingDriver = { id: string; full_name: string }

export function DriverLicenseWorkspace({ initialDrivers, canEdit, missingDrivers = [], role = "manager", assignedTasks = [], readOnlyTasks = false }: { initialDrivers: Driver[]; canEdit: boolean; missingDrivers?: MissingDriver[]; role?: string; assignedTasks?: any[]; readOnlyTasks?: boolean }) {
  ...
  async function sendReminder(target: MissingDriver | "all") {
    const key = target === "all" ? "all" : target.id
    setReminding(key)
    try {
      const body = target === "all" ? { profile_ids: missingDrivers.map((d) => d.id) } : { profile_ids: [target.id] }
      const res = await fetch("/api/transport/drivers/remind", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) })
      const result = await res.json().catch(() => null)
      if (!res.ok) throw new Error(result?.error ?? "Unable to send reminder")
      setRemindedIds((current) => new Set([...current, ...body.profile_ids]))
  ...
  return <div className="flex flex-col gap-6">
    ...
    {canEdit && missingDrivers.length > 0 && (
      <Card className="border-amber-300 bg-amber-50/60">
        <CardHeader className="flex flex-row items-center justify-between gap-4">
          <CardTitle className="flex items-center gap-2 text-amber-800"><TriangleAlert className="size-5" /> Drivers without a license on file ({missingDrivers.length})</CardTitle>
          <Button size="sm" variant="outline" disabled={reminding !== null} onClick={() => sendReminder("all")}>{reminding === "all" ? <Loader2 className="mr-1 size-4 animate-spin" /> : null}Remind all</Button>
        </CardHeader>
        <CardContent className="flex flex-col gap-2">
          {missingDrivers.map((driver) => (
            <div key={driver.id} className="flex items-center justify-between gap-3 rounded-md border bg-white px-3 py-2">
              <span className="font-medium">{driver.full_name}</span>
              {remindedIds.has(driver.id) ? (
                <span className="inline-flex items-center gap-1.5 text-xs font-medium text-emerald-600"><CheckCircle2 className="size-3.5" /> Reminder sent</span>
              ) : (
                <Button size="sm" variant="secondary" disabled={reminding !== null} onClick={() => sendReminder(driver)}>{reminding === driver.id ? <Loader2 className="mr-1 size-4 animate-spin" /> : null}Send reminder</Button>
              )}
            </div>
          ))}
        </CardContent>
