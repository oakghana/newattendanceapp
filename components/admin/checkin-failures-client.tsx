...
export function CheckinFailuresClient() {
  ...
  const [forceTarget, setForceTarget] = useState<SummaryUser | null>(null)
  ...
  const handleDeleteRecords = useCallback(async () => {
    try {
      ...
      const payload =
        deleteMode === "all"
          ? { clearAll: true, startDate, endDate }
          : { ids: Array.from(selectedIds), clearAll: false }

      const response = await fetch("/api/admin/checkin-failures", {
        method: "DELETE",
      ...
      setSuccessMsg(result.message || "Records deleted successfully")
      void fetchData()
    } catch (err) {
  ...
  }, [deleteMode, endDate, fetchData, selectedIds, startDate])

  const openForceDialog = useCallback((user: SummaryUser, action: "checkin" | "checkout") => {
    setForceTarget(user)
  ...
  const handleForceAttendance = useCallback(async () => {
    if (!forceTarget?.user_id) {
      return
    }
    ...
    try {
      ...
      const response = await fetch("/api/admin/force-attendance", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userId: forceTarget.user_id,
          action: forceAction,
      ...
      const result = await response.json()
      if (!response.ok || !result.success) {
        throw new Error(result.error || "Failed to force attendance")
      }
  ...
  }, [fetchData, forceAction, forceNote, forceTarget])
      ...
      {successMsg && (
        <Alert className="border-emerald-300 bg-emerald-50 text-emerald-800">
          ...
          <AlertDescription className="flex items-center justify-between gap-2">
            {successMsg}
            <button onClick={() => setSuccessMsg(null)} className="ml-2 text-xs underline">
              Dismiss
            </button>
          </AlertDescription>
        </Alert>
              ...
              {loading ? (
                ...
                records.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell>{row.department_name}</TableCell>
                    <TableCell>
                      <Badge variant="secondary">{formatAttemptType(row.attempt_type)}</Badge>
                    </TableCell>
                    <TableCell className="capitalize">{row.failure_reason.replace(/_/g, " ")}</TableCell>
                    <TableCell>{row.nearest_location_name}</TableCell>
                    <TableCell>
                      {row.nearest_location_distance_m != null ? `${Math.round(row.nearest_location_distance_m)}m` : "-"}
                    </TableCell>
                    <TableCell>
      ...
      <Dialog open={deleteConfirmOpen} onOpenChange={setDeleteConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {deleteMode === "all"
                ? `This will permanently delete ALL failure records in the current date range (${startDate} to ${endDate}). This cannot be undone.`
